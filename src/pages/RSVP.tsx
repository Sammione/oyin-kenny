...
import { registerGuest } from '../lib/actions'
...
export default function RSVPPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await registerGuest(name, phone, attending)
      ...
      if (result.success && result.guest) {
        ...
        navigate(`/rsvp/success?token=${result.guest.qrToken}&attending=${result.guest.attending}`)
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-stone-50 py-12 px-4 flex flex-col justify-center">
      <div className="max-w-md w-full mx-auto">
        <Link to="/" className="inline-flex items-center text-[#556B2F] hover:text-[#37461E] mb-8 font-medium">
